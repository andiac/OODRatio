...
# transform_crop_and_resize = transforms.Compose([transforms.RandomCrop(8), transforms.Resize(32, interpolation=InterpolationMode.NEAREST), transforms.ToTensor()])
...
train_loader_2 = torch.utils.data.DataLoader(datasets.CIFAR10(args.data_dir, train=True, 
                download=True, transform=transform_train), batch_size=batch_size, shuffle=True, **train_kwargs)

# ood_train_loader  = torch.utils.data.DataLoader(OE_Dataset(os.path.join(args.data_dir, "300K_random_images.npy"), 
#                 transform=transforms.ToTensor()), batch_size=batch_size, shuffle=True, **train_kwargs)

ood_train_loader = torch.utils.data.DataLoader(datasets.CIFAR10(args.data_dir, train=True, 
...
cifar100_loader  = torch.utils.data.DataLoader(datasets.CIFAR100(args.data_dir, train=False, 
                transform=transform_test, download=True), batch_size=batch_size, shuffle=False, **kwargs)
...
writer = SummaryWriter(log_dir=f"./logs/classifier_log/{args.model}_{batch_size}_{lr}_{args.noise_std}_{anneal_alpha}_{args.anneal_gamma}")
...
for e in range(EPOCH):
    ...
    cifar100_scores = get_scores(cifar100_loader)[:10000]
    ...
    cifar100_AUROC = roc_auc_score([0]*10000 + [1] * 10000, np.concatenate((test_scores, cifar100_scores)))
    ...
    print(f"epoch {e}: train_loss: {train_loss / train_N:.4f}, svhn_AUROC: {svhn_AUROC: .4f}, cifar100_AUROC: {cifar100_AUROC:.4f}, celeba_AUROC: {celeba_AUROC:.4f}, lsun_AUROC: {lsun_AUROC:.4f}")

    writer.add_scalar("train_loss", train_loss / train_N, e)
    writer.add_scalar("svhn_AUROC", svhn_AUROC, e)
    writer.add_scalar("cifar100_AUROC", cifar100_AUROC, e)
